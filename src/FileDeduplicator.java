...
public class FileDeduplicator {
    ...
    FileDeduplicator(final String filePath) {
        ...
        Preconditions.checkState(this.directory.isDirectory(), "Supplied directory does not exist");
    }
    ...
    public Collection<FileDuplicate> calculateDuplicates() {
        ...
        final FluentIterable<FileRecord> records = Files.fileTreeTraverser()
        ...
        final Multimap<HashCode, Path> dupesOnly =
                Multimaps.filterKeys(duplicateCandidates, record -> duplicateCandidates.get(record).size() > 1);

        final FluentIterable<FileDuplicate> fileDuplicates = FluentIterable.from(dupesOnly.asMap()
                .entrySet())
        ...
        System.out.println("Deduplication took " +(System.currentTimeMillis() - start) + " ms");
        return ImmutableSet.copyOf(fileDuplicates);
    }

    public static void main(String[] args) {
        final FileDeduplicator fileDeduplicator = new FileDeduplicator("/Users/amir.afghani/pgns");
    ...
    static class FileDuplicate {
        ...
        FileDuplicate(final HashCode hashCode,
                      final Collection<Path> paths) {
